...
- `tsconfig.json`의 `jsx`를 `preserve → react-jsx`로 변경(Next 16 권장). `sanity.config.ts` 등
  비-Next JSX 파일을 tsc로 typecheck하려면 자동 런타임이 필요. Sanity가 styled-components를
  peer로 요구해 deps에 추가.
...
## 백로그(이번 미구현, 의도된 부분)
- 읽기 토큰 기반 클라이언트(초안/미리보기 라우트) 배선 — 현재 공개 CDN 클라이언트만 사용.
- 스키마 `cover`(이미지) 필드는 normalize/types/queries에 아직 미매핑 — 글 렌더 라우트에서 추가.
- useRuehanix를 react-hooks v6(React Compiler) 규칙에 맞게 리팩터링(현재 warn 4종).
