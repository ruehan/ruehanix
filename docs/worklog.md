...
- 리뷰: self-review. wheel 핸들러는 shell 컴포넌트 안에 inline 으로 있어 backdrop 만 추출하면 별도 테스트 가능하나, shell 전체 마운트(dynamic + boot useEffect + useSyncExternalStore) 가 happy-dom 단위 부담이 커서 본 작업 범위 밖으로 미룸. 대신 viewModel 의 ws/gotoWs 가 Super+1~6 기존 키보드 경로로 동작 검증됨 (handlers/gotoWs + windowState/gotoWsState). ADR 0063 후속으로 폴더 Cmd+Wheel (Option D) 분리 가능.
- 가정: 사용자가 wheel 핸들러의 <1, >6 클램프·booting 가드를 코드 리뷰로 확인 (단위 테스트 미작성). deltaY 만 사용 — 좌우 wheel (trackpad horizontal) 은 잡지 않음 (의도치 않은 제스처 회피). `APP_META.name`/`CATS.label`/`localhost`/`Ctrl+K` 의 영문 유지를 의도적 디자인 어휘로 간주.
- 관련 결정: docs/decisions/0063-shell-korean-scroll-ws.md
